...
############################################################################################################################
############################ FUNCTIONS TO HANDLE PEcAn FILES ###########################################################
############################################################################################################################


#' Get a Field for PEcAn
#' 
#' 
#' An internal function that reads data from an PEcAn .nc file.  
#' 
#' @param source A \code{Source} containing the meta-data about the PEcAn source
...
getField_PEcAn <- function(source,
  ...
  if(length(all.time.intervals) > 0) { 
    ...
    if(timestep.unit != "days") stop('Currently PEcAn only accepts data with a units of time axis = "days"')
    ...
    else stop("Data doesn't appear to be daily, monthly, or yearly.  Other options are not currently supported by the PEcAn Format, but could potentially be.  So if you need this please contact the author.")
    ...
    if(time.res == "Year") {
      ...
      if(length(target.STAInfo@first.year) > 0 & length(target.STAInfo@last.year) > 0) {
        ...
        if(target.first.year != start.date.year | target.first.year != end.date.year) {
          ...
          if(target.first.year < start.date.year) stop(paste("In PEcAn requested first.year = ", target.first.year, ", but first year in data is",  start.date.year, sep = " "))
          if(target.last.year > end.date.year) stop(paste("In PEcAn requested last.year = ", target.last.year, ", but last year in data is", end.date.year, sep = " "))
      ...
      if(length(target.STAInfo@first.year) > 0 & length(target.STAInfo@last.year) > 0) {
        ...
        if(target.first.year != years.vector[1] | target.first.year != years.vector[length(years.vector)]) {
          ...
          if(target.first.year < years.vector[1]) stop(paste("In PEcAn requested first.year = ", target.first.year, ", but first year in data is", years.vector[1], sep = " "))
          if(target.last.year > years.vector[length(years.vector)]) stop(paste("In PEcAn requested last.year = ", target.last.year, ", but last year in data is", years.vector[length(years.vector)], sep = " "))
      ...
      else stop("PEcAn format requires a 'calendar' attribute on the time axis for daily data")
      proleptic <- FALSE
      ...
      while(day.counter < all.time.intervals[length(all.time.intervals)]) {
        ...
        # 360 day
          ...
          stop(paste0("Calendar", calendar, "not supported by PEcAn format in DGVMTools package"))
        }
    ...
    subannual.original.attr <- ncdf4::ncatt_get(nc = this.nc, varid = 0, attname = "PEcAn_subannual.original")
  ...
  if(!is.null(xmin) & !is.null(xmax) & !is.null(ymin) & !is.null(ymax)){
    ...
    if(verbose) message("Setting spatial extent from PEcAn attributes")
  }
    ...
    if(verbose) message(paste("No spatial extent determinable from PEcAn", file.name.nc,"file. A filler extent will be set."))
    warning(paste("No spatial extent determinable from PEcAn", file.name.nc,"file. A filler extent will be set."))
...
availableQuantities_PEcAn <- function(source, names){
...
#' Detemine PFTs present in an PEcAn source 
#' 
#' @param x  A Source objects describing a PEcAn source
...
determinePFTs_PEcAn <- function(x, variables) {
...
#' @description \code{PEcAn} - a format defined here to unify the multitude of different datasets into a common format.  It is essentially CF-compliant netCDF with a couple of extra attributes defined. 
#' Can be produced using the companion PEcAn package. 
#' 
...
PEcAn <- new("Format",
                ...
                determinePFTs = determinePFTs_PEcAn,
                
                # FUNCTION TO LIST ALL QUANTIES AVAILABLE IN A RUN
                availableQuantities = availableQuantities_PEcAn,
                
                # FUNCTION TO READ A FIELD 
                getField = getField_PEcAn,
                
                # DEFAULT GLOBAL PFTS  
                default.pfts = list(),
                
                # QUANTITIES THAT CAN BE PULLED DIRECTLY FROM LPJ-GUESS RUNS  
                quantities = PEcAn.quantities
