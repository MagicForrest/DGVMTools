...
#' Scatter plots
#' 
#' For a list of Fields, compare layers using scatter plots.  Automatically splits data into separate panels based on Source, Quantity, 
...
plotScatter <- function(fields1, # can be a Field or a list of Fields
                        fields2=fields1, # can be a Field or a list of Fields
                          layers1,
                        layers2 = layers1,
                          gridcells = NULL,
                          title = NULL,
  ...
  fields1 <- santiseFieldsForPlotting(fields1)
  if(is.null(fields1)) return(NULL)
  fields2 <- santiseFieldsForPlotting(fields2)
  if(is.null(fields2)) return(NULL)
  
  ## 3. DIMENSIONS - check the dimensions (require that all fields the same dimensions)
  
  # check the Fields for consistent dimensions
  dim.names <- santiseDimensionsForPlotting(append(fields1, fields2))
  if(is.null(dim.names)) return(NULL)
  
  ## 2. LAYERS - check the number of layers
  
  # this is special version for plotScatter since 
  plot_dt <- santiseLayersForPlottingXY(fields1, fields2, layers1, layers2)
  print(plot_dt)
  ...
  final.fields <- trimFieldsForPlottingXY(fields, layers, gridcells = gridcells)
