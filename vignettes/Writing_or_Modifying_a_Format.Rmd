...
GUESS@id
```
The @default.pfts slot is just a list of DGVMTools::Layer objects.
...
This is a function which will look at a some output files on a disk (normally output from a DGVM run) and return a list of Quantities available to read. The first argument should be the Source object.  The second argument "names", is logical and ff TRUE, the function should return a list of the ids of the quantities, otherwise it returns the list of DGVMTools::Quantity objects.  Additionaly arguments for Format-specific details can also be added.
