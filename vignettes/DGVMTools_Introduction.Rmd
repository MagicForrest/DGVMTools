...
spatial_example_plot <- plotSpatial(LAI.year.mean, 
                                    layers = c("BNE", "BINE", "BNS", "IBS", "TeBS", "TeBE", "C3G", "C4G"),
                                    text.multiplier = 3)
...
# Calculate if trees or grasss is dominant 
...
Now we formally compare the layers using the `compareLayers()` function, which also gives us some comparison metrics.  The resulting `Comparison` object can be plotting with `plotSpatialComparison()` and `plotScatterComparison()`.
...
print(plotSpatialComparison(vegC.comparison, 
                            ...
                            #legend.title = expression(pastedelta~kgC~m^-2)
                            legend.title = expression(Delta~kgC~m^-2), 
                            map.overlay = "world"))
...
The plotting functions always adds a column called `Facet` by which to facet the data.  Hovewever, you can provide the ggplot2-style
arguments "nrow" and "ncol" directly to plotSpatial().  You can also adjust the facetting after the fact with a ggplot2::facet_wrape() command. Facet layout can be adjusted as follows:
...
``` {r legends, fig.height=8, echo=TRUE}
...
spatial_example_plot <- spatial_example_plot + theme(plot.margin = margin(t = 0.2,  r = 0.2, b = 2.2, l = 0.2, unit = "cm"))
temporal_example_plot <- temporal_example_plot + theme(plot.margin = margin(t = 0.2,  r = 0.2, b = 2.2, l = 0.2, unit = "cm"))
subannual_example_plot <- subannual_example_plot + theme(plot.margin = margin(t = 0.2,  r = 0.2, b = 0.5, l = 0.2, unit = "cm"))
comparison_example_plot <- comparison_example_plot + theme(plot.margin = margin(t = 0.2,  r = 0.2, b = 0.5, l = 0.2, unit = "cm"))
