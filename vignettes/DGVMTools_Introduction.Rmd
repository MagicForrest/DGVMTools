...
Now we formally compare the layers using the `compareLayers()` function, which also gives us some comparison metrics.  The ersulting `Comparison` object can be plotting with `plotSpatialComparison()` and `plotScatterComparison()`.
...
# build a slightly reformatted version for the reference publication
...
if(require(ggpubr)) {
  ...
  combined_plot <- ggarrange(spatial_example_plot, temporal_example_plot, subannual_example_plot, comparison_example_plot, ncol = 2, nrow = 2, labels = list("(a)", "(b)", "(c)", "(d)"), font.label = list(size = 35), hjust = 0, vjust = 2.5) 
  
  # magicPlot(p = combined_plot, filename = file.path(plot_dir, "ERL_Fig2_combined"), dpi = 72, type = "pdf", height = 1200, width = 1000, family = "Sans")
  
  if(require(Cairo) & TRUE){
