...
run <- defineVegRun(run.dir = "/home/forrest/RVCToolsExamples/Example2/Run1",
                    model = "LPJ-GUESS",
                    pft.set = global.PFTs,
                    id = "ExampleRun1",
...
test1 <- addSeasonal(test)

test2 <- addVegTotals(test, c("Seasons", "Annual"), method = "mean")

sum(test1@data[,SON] - test2@data[,SON])


stop()

expandTargets("lifeform", test, global.PFTs)
