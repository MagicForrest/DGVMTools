...
for(run.dir in vegrun.list){
  ...
  for(var in this.var.list){
    ...
    # time average
    this.TA.present <- doTimeAverage.cmpd(this.full, first.year.present,last.year.present, verbose)
    ...
    if(verbose) message("Saving as a table...")
       write.table(this.TA, file = paste(run.dir, "/", var, ".TA.", first.year.present, "-", last.year.present, ".Rtable", sep =""), quote = FALSE, row.names = FALSE)
       write.table(this.SA, file = paste(run.dir, "/", var, ".SA.Rtable", sep =""), quote = FALSE, row.names = FALSE)
