...
getGFED4Monthly <- function(first.year = 1996, last.year = 2012, plot.data = FALSE, as.fraction = TRUE, return.period = "monthly",  resolution = "QD"){
  ...
  for(year in first.year:last.year){
    ...
    # because we only want the annual totals, use the monthly data use monthly data
    for(month in 1:12){
      ...
      if(tolower(return.period) == "annual" | tolower(return.periof) == "yearly") {
      if(month == 1) {
